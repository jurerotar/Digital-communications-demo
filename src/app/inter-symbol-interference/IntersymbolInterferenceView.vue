...
export default {
  ...
  data() {
    return {
      ...
      combinationsSequence: {
        values: [
          -1, -3, 3, 3, 1, -1, 3, -3, -3, 1, 3, -1, 1, 1, -3, -1, -3, 1, 1, -1,
          3, 1, -3, -3, 3, -1, 1, 3, 3, -3, -1,
        ],
        idx: 0,
      },
  ...
  mounted() {
    ...
    this.intervalId = window.setInterval(() => {
      if (this.isPlaying) {
        if (
          this.transferFunctionParams.fCutMulti_old !==
          this.transferFunctionParams.fCutMulti
        ) {
          this.transferFunctionParams.fCutMulti_old =
            this.transferFunctionParams.fCutMulti;
        ...
        const newConvolutionValue = this.binaryLevel4Signal.values
          .map((f, i) => f * this.transferFunction[i])
          .reduce((a, b) => a + b, 0);
        this.convolution.unshift(newConvolutionValue);
        // ISI oscilloscope plot
        this.dataIsiPartial.push(
          newConvolutionValue / this.transferFunctionParams.maxConvValue,
        );
        ...
        this.tsTicks = this.tsTicks.map((i) =>
          i < this.signalLength - 1 ? i + 1 : 0,
        );
        ...
        time += 0.005;
        this.binaryCounter =
          this.binaryCounter === this.binarySymbolLength - 1
            ? 0
  ...
  methods: {
    ...
    nextBinaryValue() {
      if (this.binaryCounter === this.binarySymbolLength - 1) {
        this.combinationsSequence.idx =
          this.combinationsSequence.idx ===
          this.combinationsSequence.values.length - 1
            ? 0
            : this.combinationsSequence.idx + 1;
        this.currentlyReturns =
          this.combinationsSequence.values[this.combinationsSequence.idx];
      } else {
    ...
    recalculateTicks(reset) {
      ...
      if (reset) {
        this.tsTicks = [
          ...Array(
            Math.floor(this.signalLength / this.binarySymbolLength),
          ).fill(0),
        ]; // reset ticks
        for (let i = 0; i < this.tsTicks.length; i++) {
          this.tsTicks[i] =
            (i +
              this.transferFunctionParams.tOffset / this.binarySymbolLength) *
            this.binarySymbolLength;
        ...
        for (let i = 0; i < this.tsTicks.length; i++) {
          const dtOffset =
            this.transferFunctionParams.tOffset -
            this.transferFunctionParams.tOffset_old;
          this.tsTicks[i] = this.tsTicks[i] - dtOffset;
        }
        this.tsTicks.map((i) =>
          i >= this.signalLength - 1 ? i - this.signalLength : i,
        ); // cut ticks that are out of range
        this.transferFunctionParams.tOffset_old =
          this.transferFunctionParams.tOffset;
      }
      // eye diagram ticks
      ...
      for (let i = 0; i < 2; i++) {
        this.isiTicks[i] =
          this.transferFunctionParams.tOffset + i * this.binarySymbolLength;
      ...
      const max = Math.floor(
        (this.signalLength - this.transferFunctionParams.tOffset) /
          this.binarySymbolLength,
      );
      for (let i = min; i <= max; i++) {
        this.transferFunctionParams.ticks.unshift(
          this.transferFunctionParams.tOffset + i * this.binarySymbolLength,
        );
        this.transferFunctionParams.tickNames.unshift(
          'ts'.concat(i.toString()),
        );
      }
    },
    /**
    ...
    updateTransferFunctionParams() {
      ...
      this.transferFunctionParams.tOffset =
        1.5 /
        (this.transferFunctionParams.fCut *
          this.transferFunctionParams.fCutMulti);
    ...
    updateTransferFunction() {
      ...
      this.transferFunction = this.transferFunction.map((i, t) => {
        return (
          Math.sin(2 * Math.PI * fCut * multi * (t - tOffset)) /
          (2 * Math.PI * fCut * multi * (t - tOffset))
        );
      });
      this.transferFunction = this.transferFunction.map((i) =>
        Number.isNaN(i) ? 1 : i,
      ); // 1/(t-tOffset); t=tOffset => NaN
      ...
      const worstCaseSignal = [...Array(this.signalLength).fill(0)].map(
        (tf, t) => {
          return t % this.binarySymbolLength === this.binarySymbolLength - 1
            ? 3
            : 0;
        },
      );
      ...
      this.transferFunctionParams.maxConvValue =
        worstCaseConv[
          Math.round(
            this.signalLength - 1 - this.transferFunctionParams.tOffset,
          )
        ];
    },
    togglePlay() {
