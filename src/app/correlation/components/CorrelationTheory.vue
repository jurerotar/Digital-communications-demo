<template>
  <div class="flex flex-col gap-4">
    ...
    <AppParagraph>
      ...
      <KatexEquation inline>
        {{ 'y(t + τ)' }}
      </KatexEquation>, ki sta
      periodična s periodo
      <KatexEquation inline>
        {{ 'T' }}
      </KatexEquation>,
      je definirana kot srednja vrednost njunega produkta: <br>
      <KatexEquation>
        {{ 'R_{xy}=\\overline{x(t)y(t)}={1\\over T}\\int_Tx(t)y(t)dt' }}
      </KatexEquation>
    ...
    <AppParagraph>
      Če naredimo korelacijo med prvim signalom
      <KatexEquation inline>
        {{ 'x(t)' }}
      </KatexEquation>
      in za
      ...
      časovno zamaknjenim drugim signalom
      <KatexEquation inline>
        {{ 'y(t + τ)' }}
      </KatexEquation>,
      je korelacija odvisna od zamika
      <KatexEquation inline>
        {{ 'τ' }}
      </KatexEquation>, oziroma je funkcija
      zamika
      <KatexEquation inline>
        {{ 'τ' }}
      </KatexEquation>.
      ...
      <KatexEquation>
        {{ 'R_{xy}(τ)=\\overline{x(t)y(t + τ)}={1\\over T}\\int_Tx(t)y(t + τ)dt' }}
      </KatexEquation>
      Korelacijska funkcija ima največjo vrednost pri tistem zamiku
      <KatexEquation inline>
        {{ 'τ' }}
      </KatexEquation>, pri katerem sta si signala med
      seboj najbolj podobna. Kadar je korelacijska funkcija dveh signalov enaka 0 pri vsakem zamiku
      <KatexEquation inline>
        {{ 'τ' }}
      </KatexEquation>, pravimo, da sta signala med seboj nekorelirana.
    </AppParagraph>
  </div>
</template>
