...
import type { PulseShape } from '@interfaces/spectrum';
import { type MaybeRef, type Ref, ref, toValue, watchEffect } from 'vue';

type PulseGenerators = Record<
  PulseShape,
  Record<
    'baseSignalGenerator' | 'spectrumSignalGenerator',
    (frequency: number, baseSignalValue: number) => number
  >
>;
...
const pulseGenerators: PulseGenerators = {
  cos: {
    baseSignalGenerator: (frequency, baseSignalValue: number) => {
      return (
        (-Math.cos(Math.PI * baseSignalValue * frequency * 0.01) - 1) *
        unitBox((baseSignalValue * frequency * 0.01) / 2, [-0.5, 0.5])
      );
    },
    spectrumSignalGenerator: (frequency, baseSignalValue: number) => {
      return (
        (-Math.cos(Math.PI * baseSignalValue * frequency * 0.05) - 1) *
        unitBox((baseSignalValue * frequency * 0.05) / 2, [-0.5, 0.5])
      );
    },
  },
  square: {
  ...
  gauss: {
    baseSignalGenerator: (frequency, baseSignalValue: number) => {
      return (
        Math.E ** (-2 * (baseSignalValue * 0.0165 * frequency) ** 2) *
        -unitBox(baseSignalValue * frequency * 0.0005, [-0.5, 0.5])
      );
    },
    spectrumSignalGenerator: (frequency, baseSignalValue: number) => {
      return (
        Math.E ** (-2 * (baseSignalValue * 0.028 * frequency) ** 2) *
        -unitBox(baseSignalValue * frequency * 0.0005, [-0.5, 0.5])
      );
    },
  },
  sinc: {
    baseSignalGenerator: (frequency, baseSignalValue: number) => {
      return baseSignalValue === 0
        ? -1
        : (-Math.sin(baseSignalValue * frequency * 0.062) /
            (baseSignalValue * frequency * 0.062)) *
            unitBox(baseSignalValue * frequency * 0.0033, [-0.5, 0.5]);
    },
    spectrumSignalGenerator: (frequency, baseSignalValue: number) => {
      return baseSignalValue === 0
        ? -1
        : (-Math.sin(baseSignalValue * frequency * 0.152) /
            (baseSignalValue * frequency * 0.152)) *
...
const calculateSignals = (
  ...
  const baseSignalValues: number[] = [
    ...arrayOfHalfSignalSize.map((el) => el * -1 - 1).reverse(),
    ...arrayOfHalfSignalSize,
  ];
  const { baseSignalGenerator, spectrumSignalGenerator } =
    pulseGenerators[pulseShape];
  const shapedSignal = takeMiddleValues(
    baseSignalValues.map((t: number) => baseSignalGenerator(frequency, t)),
  );
  const shapedSpectrumSignal = baseSignalValues.map((t: number) =>
    spectrumSignalGenerator(frequency, t),
  );
  return {
    shapedSignal,
